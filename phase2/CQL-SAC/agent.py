...
class CQLSAC(nn.Module):
    ...
    def __init__(self,
        ...
        self.cql_weight_start = 0.0
        self.cql_weight_end = cql_weight  # 还是用你外面传进来的cql_weight=1.0
        self.cql_weight_warmup_epochs = 50  # 你可以自己调节这个值
        self.current_epoch = 0  # 记得每个epoch手动+1
    ...
    def get_action(self, state, eval=False):
        ...
        # ⚠️ 添加维度检查，如果不对直接报错
        assert state.shape[0] == self.state_size, f"Expected state dim {self.state_size}, got {state.shape[0]}"
    ...
    def learn(self, experiences):
        ...
        progress = min(1.0, self.current_epoch / self.cql_weight_warmup_epochs)
        adaptive_cql_weight = self.cql_weight_start + progress * (self.cql_weight_end - self.cql_weight_start)
        ...
        cql1_scaled_loss = ((torch.logsumexp(cat_q1 / self.temp, dim=1).mean() * adaptive_cql_weight * self.temp) - q1.mean()) * adaptive_cql_weight
        cql2_scaled_loss = ((torch.logsumexp(cat_q2 / self.temp, dim=1).mean() * adaptive_cql_weight * self.temp) - q2.mean()) * adaptive_cql_weight
