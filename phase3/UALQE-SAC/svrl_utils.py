...
def softimp_ua(qmat_tensor, mask=None, rank=None, n_iter=10, tol=1e-3, verbose=False):
    """
    SoftImpute with optional external mask support (for uncertainty-aware masking).
    
    Args:
        qmat_tensor: [B, K] float tensor (on CPU or GPU)
        mask: [B, K] binary float tensor (1=observed, 0=missing). If None, use random mask.
        mask_prob: float, if mask is None, randomly mask this proportion of entries
        rank: int, number of singular values to retain (if None, use full rank)
    ...
    for i in range(n_iter):
        ...
        if rank is not None:
            S[rank:] = 0.0
        Z_new = (U @ torch.diag(S) @ Vh)
        Z_new = Z_new * mask + qmat_masked * (1 - mask)  # overwrite only masked entries
